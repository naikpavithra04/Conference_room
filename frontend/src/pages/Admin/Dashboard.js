...
const Dashboard = () => {
  const cards = [
    {
      title: "Manage Rooms",
      desc: "Add, edit and manage conference rooms",
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-300 mt-2">
            Manage your conference room booking system efficiently
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {cards.map((card, index) => (
            <Link
              key={index}
              to={card.link}
              className="group bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 shadow-lg hover:shadow-2xl transition duration-300 transform hover:-translate-y-1"
            >
              <div
              ...
              <div className="mt-4 text-blue-300 font-medium text-sm opacity-0 group-hover:opacity-100 transition">
                Open →
              </div>
            </Link>
          ))}
